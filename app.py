...
if "scores" in st.session_state:
    ...
    st.info(f"🎯 Detected JD Domain: **{domain.replace('_', ' ').title()}** — scoring weights adjusted accordingly")
    ...
    def color_cgpa(v):
        try:
            return "color: green; font-weight: 600;" if float(v) >= 8 else ""
        except Exception:
            return ""
    ...
    pick = st.selectbox("Select a candidate", scores["candidate_id"].tolist())
